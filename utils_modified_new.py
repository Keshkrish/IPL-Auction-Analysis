...
def get_batting_stats(player_pkey,matches_list=None):
    if matches_list==None:
        ...
        df = data_batting_scorecard[(data_batting_scorecard['player_pkey'] == player_pkey) & (data_batting_scorecard['status'] == 'YES') & (data_batting_scorecard['match_pkey'].isin(matches_list))]
    if len(df)==0:
        return [0, 0, 0, 0]
    else:
        runs = sum(df['runs'])
...
def get_bowling_stats(player_pkey,matches_list=None):
    if matches_list==None:
        df = data_bowling_scorecard[data_bowling_scorecard['player_pkey'] == player_pkey]
    else:
        df=data_bowling_scorecard[(data_bowling_scorecard['player_pkey']==player_pkey)& (data_bowling_scorecard['match_pkey'].isin(matches_list))]
    if len(df)==0:
        return [0, 0, 0, 0, 0]
    else:
        runs = sum(df['runs'])
...
'''
def get_CBR(player_pkey, phase='powerplay',matches_list=None):
    if matches_list==None:
        ...
        if phase == 'powerplay':
            df = data_bbb[(data_bbb['bowler_pkey'] == player_pkey) & (data_bbb['over_num'] <= 6) & (data_bbb['match_pkey'].isin(matches_list))]
        elif phase == 'death':
            df = data_bbb[(data_bbb['bowler_pkey'] == player_pkey) & (data_bbb['over_num'] > 15) & (data_bbb['match_pkey'].isin(matches_list))]
...
def get_BP(player_pkey, phase='powerplay',matches_list=None):
    if matches_list==None:
        ...
        if phase == 'powerplay':
            df = data_bbb[(data_bbb['batsman_pkey'] == player_pkey) & (data_bbb['over_num'] <= 6)& (data_bbb['match_pkey'].isin(matches_list))]
        elif phase == 'death':
            df = data_bbb[(data_bbb['batsman_pkey'] == player_pkey) & (data_bbb['over_num'] > 15)& (data_bbb['match_pkey'].isin(matches_list))]
